...
import com.corporation.manager.domain.announcementDTO;
import com.corporation.manager.domain.announcementVO;
import com.corporation.manager.domain.corporation_announcement;
...
public class CorporationInforAction extends ActionSupport {
	...
	public String showMember() throws UnsupportedEncodingException, InterruptedException {
		// 检查权限
	...
	public void agreeApply() throws UnsupportedEncodingException, IOException {
		System.out.println("同意申请的会员id+" + currMember);
	...
	public String getAnnouncementByPage() {
		announcementDTO annoD = new announcementDTO();
		annoD.setCurrCorporation(member_corporation);
		annoD.setQueryEndTime(queryEndTime);
		annoD.setQueryStartTime(queryStartTime);
		annoD.setQuerySort(timeSort);
		annoD.setQueryName(queryName);
		annoD.setCurrPage(currPage);
		announcementVO annoV = corporationInforService.getAnnouncement(annoD, 10.0);
		Gson gson = new Gson();
		System.out.println(gson.toJson(annoV));
		ActionContext.getContext().getValueStack().set("annoVO", annoV);
		return "getAnnoSuccess";
	}

	public String skipToShowAnno() {
		announcementDTO annoD = new announcementDTO();
		annoD.setCurrCorporation(member_corporation);
		annoD.setQueryEndTime(queryEndTime);
		annoD.setQueryStartTime(queryStartTime);
		...
		annoD.setQueryName(queryName);
		annoD.setCurrPage(currPage);
		announcementVO annoV = corporationInforService.getAnnouncement(annoD, 5.0);
		Gson gson = new Gson();
		System.out.println(gson.toJson(annoV));
		ActionContext.getContext().getValueStack().set("annoVO", annoV);
		return "showAnnoSuccess";
	}

	// ----------------------------------发布公告------------------------------------------------------------------

	public void addAnnouncement() throws IOException {
		corporation_announcement ca = new corporation_announcement();
		if (announcement_imageFileName != null) {
			String filename = uuid.getUuid()
					+ announcement_imageFileName.substring(announcement_imageFileName.indexOf("."));
			String path = ServletActionContext.getServletContext().getRealPath("/upload/announcementImage/" + filename);
			File img = new File(path);
			FileUtils.copyFile(announcement_image, img);
			ca.setAnnouncement_image(filename);

		}
		if (announcement_filesFileName != null) {
			String filename = uuid.getUuid()
					+ announcement_filesFileName.substring(announcement_filesFileName.indexOf("."));
			String path = ServletActionContext.getServletContext().getRealPath("/upload/announcementFile/" + filename);
			File files = new File(path);
			FileUtils.copyFile(announcement_files, files);
			ca.setAnnouncement_files(filename);
		}
		ca.setAnnouncement_name(announcement_name);
		ca.setAnnouncement_belone(member_corporation);
		ca.setAnnouncement_content(announcement_content);
		ca.setAnnouncement_author(corporationInforService
				.getUserById((String) ServletActionContext.getContext().getSession().get("user_id")).getUser_name());
		ca.setAnnouncement_id(uuid.getUuid());
		ca.setAnnouncement_gmt_create(TimeUtil.getStringDay());
		ca.setAnnouncement_gmt_modified(TimeUtil.getStringSecond());
		Gson gson = new Gson();
		System.out.println(gson.toJson(ca));
		corporationInforService.addAnnouncement(ca);
	}

	// ----------------------------------------删除公告----------------------------------------------------

	public void deleteAnnouncement() {
		System.out.println("id:" + announcement_id);
		corporation_announcement ano = corporationInforService.getAnnouncementById(announcement_id);

		String imgPath = ServletActionContext.getServletContext()
				.getRealPath("/upload/announcementImage/" + ano.getAnnouncement_image());

		String filePath = ServletActionContext.getServletContext()
				.getRealPath("/upload/announcementFile/" + ano.getAnnouncement_files());

		File img = new File(imgPath);
		File file = new File(filePath);
		if (img.exists()) {
			System.out.println("图片存在并删除");
			img.delete();
		}
		if (file.exists()) {
			System.out.println("文件存在并删除");
			file.delete();

		}
		corporationInforService.deleteAnnouncement(announcement_id);
	}

	// ------------------------------------------------------通过id获得公告---------------------------------------------------------s
	public void getAnnouncementById() throws IOException, InterruptedException {
		corporation_announcement ano = corporationInforService.getAnnouncementById(announcement_id);
		Gson gson = new Gson();
		String result = gson.toJson(ano);
		System.out.println(result);
		HttpServletResponse response = ServletActionContext.getResponse();
		response.setContentType("text/html;charset=utf-8");
		PrintWriter pw = response.getWriter();
	...
	private File announcement_image;
	private String announcement_imageFileName;
	private String announcement_imageContentType;
	private String announcement_filesFileName;
	private String announcement_filesContentType;
	private File announcement_files;
	...
	public File getAnnouncement_image() {
		return announcement_image;
	}

	public void setAnnouncement_image(File announcement_image) {
		this.announcement_image = announcement_image;
	}

	public String getAnnouncement_imageFileName() {
		return announcement_imageFileName;
	}

	public void setAnnouncement_imageFileName(String announcement_imageFileName) {
		this.announcement_imageFileName = announcement_imageFileName;
	}

	public String getAnnouncement_imageContentType() {
		return announcement_imageContentType;
	}

	public void setAnnouncement_imageContentType(String announcement_imageContentType) {
		this.announcement_imageContentType = announcement_imageContentType;
	}

	public String getAnnouncement_filesFileName() {
		return announcement_filesFileName;
	}

	public void setAnnouncement_filesFileName(String announcement_filesFileName) {
		this.announcement_filesFileName = announcement_filesFileName;
	}

	public String getAnnouncement_filesContentType() {
		return announcement_filesContentType;
	}

	public void setAnnouncement_filesContentType(String announcement_filesContentType) {
		this.announcement_filesContentType = announcement_filesContentType;
	}

	public File getAnnouncement_files() {
		return announcement_files;
	}

	public void setAnnouncement_files(File announcement_files) {
		this.announcement_files = announcement_files;
	}
