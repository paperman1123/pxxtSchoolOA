...
public class CorporationInforServiceImpl implements CorporationInforService {
	...
	// ----------------------------------分页获得公告----------------------------------------------------------
	@Override
	public announcementVO getAnnouncement(announcementDTO annoD, double pageSize) {
		// TODO Auto-generated method stub

		int count = corporationInforDao.getAnnouncementCount(annoD);
		double countSize = (double) count;
		int totalPage = (int) Math.ceil(countSize / pageSize);
		List<corporation_announcement> list = corporationInforDao.getAnnouncementByPage(annoD, pageSize);
		announcementVO annoV = new announcementVO();
		annoV.setCurrCorporation(annoD.getCurrCorporation());
		annoV.setCurrPage(annoD.getCurrPage());
		annoV.setPageSize((int) pageSize);
		annoV.setQueryEndTime(annoD.getQueryEndTime());
		annoV.setQueryStartTime(annoD.getQueryStartTime());
		annoV.setQueryName(annoD.getQueryName());
		annoV.setQuerySort(annoD.getQuerySort());
		annoV.setTotalPage(totalPage);
		annoV.setTotalSize(count);
		annoV.setAnnouncement(list);
		return annoV;
	}
	...
	public void addAnnouncement(corporation_announcement ca) {
		// TODO Auto-generated method stub
		corporationInforDao.addAnnouncement(ca);
	}

	@Override
	public void deleteAnnouncement(String announcement_id) {
		// TODO Auto-generated method stub
		corporationInforDao.deleteAnnouncement(announcement_id);
	}
	...
	public corporation_announcement getAnnouncementById(String announcement_id) {
		...
		corporation_announcement ano = corporationInforDao.getAnnouncementById(announcement_id);
		return ano;
	}
}
